...
def sniff_packets():
    ...
    while True:
        ...
        # Message from a socket is a tuple (data in bytes, address of socket sending data)
        tcp_ip_packet = s.recvfrom(65565)
        packet = tcp_ip_packet[0]

        # First 20 characters is IP header - prefix to an IP packet which contains information about version, source IP, destination IP, time-to-live, etc...
        iph_packed = packet[:20]
        iph = unpack('!BBHHHBBH4s4s', iph_packed)
        iph_size = parse_ip_packet(iph)

        # Start at end of IP header and get the next 20 characters
